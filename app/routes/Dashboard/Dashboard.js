...
const Dashboard = (props) => {
	const { fixingRego, updateState, goBack, logVisit, showVisitModal, checkValidVisitDate, giveWashCard, enterButtonTapped, enterWithPromoButtonTapped, addDetails } = props;
	return <View style={styles.container}>
		...
		<View style={{ flex: 1, marginBottom: width(5), marginLeft: width(12.5) }}>
			<View style={{ height: width(12), flexDirection: 'row' }}>
                ...
                <View style={{ flex: 1, flexDirection: 'row', paddingTop: width(3), paddingBottom: width(3), marginRight: width(2) }}>
					<TouchableOpacity style={styles.nameButton} onPress={addDetails.bind(this)}>
						<Text style={[styles.nameButtonText, { fontSize: 20 }]}>{props.detailsInfo}</Text>
					</TouchableOpacity>
				</View>
				<View style={{ flex: 5, flexDirection: 'row', paddingTop: width(3), paddingBottom: width(3) }}>
			...
			<View style={{ flex: 1 }}>
				<View style={[styles.tabView, { paddingBottom: props.selectedTab === 1 || props.selectedTab === 2 ? 0 : width(3) }]}>
					...
					{props.selectedTab === 2 ? <View style={{ height: width(12), justifyContent: 'center' }}>
						<TouchableOpacity style={styles.enterCarWashButton} onPress={() => checkValidVisitDate()}>
							<Text style={styles.backButtonText}>Enter Car Wash</Text>
						</TouchableOpacity>
                    </View> : null}
                    {props.selectedTab === 1 ? <View style={{ flex: 1, justifyContent: 'center', flexDirection: 'row', height: width(12)}}>
						<TouchableOpacity style={styles.enterButton} onPress={() => checkValidVisitDate()}>
							<Text style={styles.backButtonText}>Enter</Text>
						</TouchableOpacity>
                        <TouchableOpacity style={styles.enterWithPromoButton} onPress={() => enterWithPromoButtonTapped()}>
							<Text style={styles.backButtonText}>Enter With Promo</Text>
						</TouchableOpacity>
					</View> : null}
				</View>
...
Dashboard.propTypes = {
    ...
    enterButtonTapped: PropTypes.func.isRequired,
    enterWithPromoButtonTapped: PropTypes.func.isRequired,
