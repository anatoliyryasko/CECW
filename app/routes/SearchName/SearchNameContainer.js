...
class SearchNameContainer extends Component {
    ...
    regoSelected(num, userstate1) {
        this.setState({ selectedTab: num, modalVisible: false });
        if (this.state.users[this.state.itemKey].washcodes.length >= 1)
            this.state.userstate1 = 0;
        else 
            this.state.userstate1 = 1;
        this.props.navigation.navigate('Dashboard', { user: this.state.users[this.state.itemKey], regoNumber: num, userstate: this.state.userstate1, username: this.state.username, password: this.state.password, locationID: this.state.locationID });
    }
    
    goToDashboard() {
        this.setState({ isLoading: true });
        lookupIdentifier(this.state.regoName, this.state.username, this.state.password).then( response => {
            let users = [];
            let tempKey = 0;
            for ( ; ; tempKey += 1) {
            ...
            let number = users[0].vehicleRegistrations.length;
            if (users[0].washcodes.length >= 1)
                this.state.userstate1 = 0;
            else 
                this.state.userstate1 = 1;
            this.props.navigation.navigate('Dashboard', { user: users[0], regoNumber: number - 1, userstate: this.state.userstate1, username: this.state.username, password: this.state.password, locationID: this.state.locationID });
    ...
    addNewRegoToUser(flag) {
        ...
        addRego(param, this.state.username, this.state.password).then( response => {
            if (response.result === true) {
                lookupIdentifier(this.state.keyword, this.state.username, this.state.password).then( response => {
                    this.setState({ isLoading: false });
                    ...
                    <View style={styles.modalContainer}>
                        <View style={styles.modalContent}>
                            <View style={{ height: 15, justifyContent: 'center' }}>
                                <Text style={{ textAlign: 'center' }}>Select Rego</Text>
                            </View>
                                ...
                                <View style={styles.modalTextField}>
                                    <ScrollView style={{ flex: 1, padding: 10 }}>
                                        {this.state.users[this.state.itemKey].vehicleRegistrations.map((rego, key) => {
                                            return (
                                                <TouchableOpacity style={{ height: height(7), justifyContent: 'center' }} onPress={() => this.regoSelected(key, 0)}>
